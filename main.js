...
Elm.Ms = Elm.Ms || {};
Elm.Ms.make = function (_elm) {
   "use strict";
   _elm.Ms = _elm.Ms || {};
   if (_elm.Ms.values)
   return _elm.Ms.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var renderTile = F2(function (_v2,
   contents) {
      return function () {
         switch (_v2.ctor)
         {case "_Tuple2":
            return A2($Svg.rect,
              _L.fromArray([$Svg$Attributes.x(toPx(_v2._0))
                           ,$Svg$Attributes.y(toPx(_v2._1))
                           ,$Svg$Attributes.width("15px")
                           ,$Svg$Attributes.height("15px")]),
              function () {
                 switch (contents.ctor)
                 {case "Bomb":
                    return _L.fromArray([$Html.text("bomb")]);
                    case "Neighbors":
                    return _L.fromArray([$Html.text($Basics.toString(contents._0))]);}
                 return _L.fromArray([]);
              }());}
         _U.badCase($moduleName,
   ...
   var render = function (model) {
      return $Svg.svg(_L.fromArray([]))(A2($Dict.foldl,
      concatMap,
      _L.fromArray([]))(A2($Dict.map,
      renderTile,
      model.tiles)));
   };
   ...
   _v9) {
      return function () {
         ...
         {case "_Tuple2": return A2(f,
              ...
              _v9._1);}
         _U.badCase($moduleName,
         "on line 53, column 18 to 23");
      }();
   });
   var tupleMap = F2(function (f,
   _v13) {
      return function () {
         switch (_v13.ctor)
         {case "_Tuple2":
            return {ctor: "_Tuple2"
                   ...
                   ,_1: f(_v13._0)};}
         _U.badCase($moduleName,
         "on line 50, column 21 to 29");
      }();
   });
   var combine = F3(function (f,
   l1,
   l2) {
      return $List.concat($List.map(A2($Basics.flip,
      $List.map,
      l2))(A2($List.map,f,l1)));
   });
   var addTile = F2(function (_v17,
   map) {
      return function () {
         switch (_v17.ctor)
         {case "_Tuple2":
            return A3($Dict.insert,
              ...
              map);}
         _U.badCase($moduleName,
         "on line 44, column 33 to 58");
      }();
   });
   var range = function (n) {
      return _L.range(0,n - 1);
   };
   var cond = F3(function (b,t,f) {
      return b ? t : f;
   });
   var width = 1;
   var height = 1;
   ...
   var tiles = function () {
      ...
      var list = function () {
         ...
         _U.badCase($moduleName,
         "on line 37, column 21 to 58");
      }();
      return A2($List.map,
      makeTile,
      list);
   }();
   var makeMap = A2($List.foldl,
   addTile,
   $Dict.empty)(A2($List.map2,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2"
             ,_0: v0
             ,_1: v1};
   }),
   tiles)(spread(combine(F2(function (v0,
   v1) {
      return {ctor: "_Tuple2"
             ,_0: v0
             ,_1: v1};
   })))(tupleMap(range)({ctor: "_Tuple2"
                        ,_0: width
                        ,_1: height}))));
   var init = {_: {}
              ,tiles: makeMap};
   var Idle = {ctor: "Idle"};
   var updates = $Signal.mailbox(Idle);
...
Elm.Random.Float.make = function (_elm) {
   ...
   if (_elm.Random.Float.values)
   return _elm.Random.Float.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Random.Float",
   $Basics = Elm.Basics.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Random$Extra = Elm.Random.Extra.make(_elm);
   var normal = F3(function (start,
   end,
   standardDeviation) {
      return function () {
         var normalDistribution = F3(function (mean,
         stdDev,
         x) {
            return _U.eq(stdDev,
            0) ? x : function () {
               ...
               return scale * Math.pow($Basics.e,
               0 - exponent);
            }();
         });
         return A2($Random$Extra.map,
         A2(normalDistribution,
         (end - start) / 2,
         standardDeviation),
         A2($Random.$float,start,end));
      }();
   });
   var standardNormal = A3(normal,
   $Basics.toFloat($Random.minInt) + 1,
   $Basics.toFloat($Random.maxInt),
   1);
   var gaussian = normal;
   var unitRange = A2($Random.$float,
   -1,
   1);
   ...
   $Basics.toFloat($Random.minInt),
   $Basics.toFloat($Random.maxInt));
   _elm.Random.Float.values = {_op: _op
                              ,anyFloat: anyFloat
